...
!function($) {
	...
	var JMLayoutBuilder = window.JMLayoutBuilder = window.JMLayoutBuilder || {
		...
		initLayout: function(){
			...
			jmlayout
				.find('.jm_layoutbuilder_build')
				.on('click', 'li', function(){
					
					if($(this).hasClass('jm_layoutbuilder_build_responsive')){
						...
						screens.find('[data-screen]:eq(1)').removeClass('active').trigger('click');
					} else if($(this).hasClass('jm_layoutbuilder_build_positions')){
						...
						$('.jm_layoutbuilder_build_pos_tab').addClass('active').siblings().removeClass('active');
						...
						preview.removeClass('jm_layoutbuilder_build_res').removeClass('jm_layoutbuilder_build_pos').addClass('jm_layoutbuilder_build_hide');
			...
			screens.on('click', '.btn', function(e){
				e.preventDefault();
		...
		alert: function(msg, type, title, placeholder){
			...
			alrt.alert();
			setTimeout(function(){ alrt.alert('close') }, 6000);
		},
		...
		saveAssigns: function(callback){
			...
			JMLayoutBuilder.submit({
				...
				$('#jm_layoutbuilder_assigns').removeClass('changed');
		...
		getLayoutData: function(){
			var json = {},
				...
				jblocks = jcontainer.find('.ui-sortable > .jm_layoutbuilder_block').not('#jm_layoutbuilder_excluded_blocks > .jm_layoutbuilder_block'),
				...
				if(bdhtml){
					...
					preview
						.find('.jm_layoutbuilder_block').not(subBlocks).each(function(){
							var block = $(this);
							block.prepend('<span class="sort-handle hasTooltipLeft" title="' + JMLayoutBuilder.lang.PLG_SYSTEM_JMFRAMEWORK_SORT_BLOCKS + '"><i class="icon-move"></i></span>');
							
							var mainblock = block.find('[data-block-visible]');
							if(mainblock.length > 0) {
								
								block.prepend('<span class="block-hide hasTooltipLeft" title="' + JMLayoutBuilder.lang.PLG_SYSTEM_JMFRAMEWORK_HIDE_BLOCK + '"><i class="icon-eye-open"></i></span>')
								.prepend('<span class="block-show hasTooltipLeft" title="' + JMLayoutBuilder.lang.PLG_SYSTEM_JMFRAMEWORK_SHOW_BLOCK + '"><i class="icon-eye-close"></i></span>')
					...
					preview
						...
						jflxbs.each(function(){
							...
							$(flexiblock).closest('.jm_layoutbuilder_block').prepend('<span class="fullwidth-off hasTooltipLeft" title="' + JMLayoutBuilder.lang.PLG_SYSTEM_JMFRAMEWORK_FULLWIDTH_OFF + '"><i class="icon-list"></i></span>')
							.prepend('<span class="fullwidth-on hasTooltipLeft" title="' + JMLayoutBuilder.lang.PLG_SYSTEM_JMFRAMEWORK_FULLWIDTH_ON + '"><i class="icon-grid-2"></i></span>')
		...
		initSortable: function(){
			...
			var maincol = $('.jm_layoutbuilder_block').first().parent();
