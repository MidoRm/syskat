...
class PlgContentSige extends JPlugin
{
            ...
            if (!empty($imagesLoaded)) {
                ...
                if ($this->pluginParameters['copyright']) {
                    if ((!$this->pluginParameters['single'] || ($this->pluginParameters['single'] && !$singleYes)) && !$this->pluginParameters['list'] && !$this->pluginParameters['word']) {
                        $html .= '<p class="sige_small"><a href="https://joomla-extensions.kubik-rubik.de/" title="SIGE - Simple Image Gallery Extended - Kubik-Rubik Joomla! Extensions" target="_blank">Simple Image Gallery Extended</a></p>';
                    }
                }
            }
    ...
    private function htmlImage($image, &$html, $hidden, &$fileInfo, $a)
    {
        if (!empty($image)) {
            ...
            if ($this->pluginParameters['list'] && !$this->pluginParameters['word']) {
                ...
                $html .= '<li class="sige_cont_' . $this->sigCount;
                ...
                if ($this->pluginParameters['view'] == 7) {
                    ...
                    $html .= ' sige_swipe_single_' . $swipeClassId;
                }

                $html .= '"><span class="sige_thumb">';
            }
    ...
    private function setImageInformation(&$fileInfo)
    {
        ...
        $this->imageInfo = array_map('htmlspecialchars', $this->imageInfo);
    }
    ...
    private function htmlImageHidden(&$html)
    {
        if (!$this->pluginParameters['noslim']) {
            if ($this->pluginParameters['watermark']) {
                $html .= '<span class="sige_hidden"><a href="' . $this->liveSite . $this->rootFolder . $this->imagesDir . '/wm/' . $this->imageInfo['image_hash'] . '"';
            } else {
                if ($this->pluginParameters['resize_images']) {
                    $html .= '<span class="sige_hidden"><a href="' . $this->liveSite . $this->rootFolder . $this->imagesDir . '/resizedimages/' . $this->image . '"';
                } else {
                    $html .= '<span class="sige_hidden"><a href="' . $this->liveSite . $this->rootFolder . $this->imagesDir . '/' . $this->image . '"';
                }
            }
